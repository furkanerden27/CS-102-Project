...
public class LoadGameScreen implements Screen {
    ...
    private float tableX, tableY, tableWidth, tableHeight;
    ...
    public void show() {
        ...
        tableX = panelX + 20f;
        tableY = panelY + 70f;
        tableWidth = PANEL_W - 40f;
        tableHeight = PANEL_H - 150f;
    ...
    private void drawTableRows() {
        ...
        Rectangle clip = new Rectangle(tableX, tableY, tableWidth, tableHeight);
        ...
        if (com.badlogic.gdx.scenes.scene2d.utils.ScissorStack.pushScissors(scissors)) {
            ...
            float headerY = tableY + tableHeight - HEADER_H;
            sr.begin(ShapeRenderer.ShapeType.Filled);
            sr.setColor(0.20f, 0.15f, 0.28f, 0.95f);
            sr.rect(tableX, headerY, tableWidth, HEADER_H);
            sr.end();
            ...
            for (int i = 0; i < visibleCount; i++) {
                ...
                sr.rect(tableX, rowY, tableWidth, ROW_H);
                sr.end();
            }
    ...
    private void drawTexts() {
        ...
        float colLevel = tableX + tableWidth * 0.45f;
        float colHP    = tableX + tableWidth * 0.60f;
        float colGold  = tableX + tableWidth * 0.75f;
        ...
        if(loading){
            ...
            bodyFont.draw(batch, "Loading...", cx - layout.width / 2f, tableY + tableHeight / 2f);
        } else if (errorMsg != null) {
            bodyFont.setColor(Color.RED);
            layout.setText(bodyFont, errorMsg);
            bodyFont.draw(batch, errorMsg, cx - layout.width / 2f, tableY + tableHeight / 2f);
        } else if (saves.isEmpty()) {
            bodyFont.setColor(COL_TEXT);
            String msg = "No saved games found.";
            layout.setText(bodyFont, msg);
            bodyFont.draw(batch, msg, cx - layout.width / 2f, tableY + tableHeight / 2f);
        } else {
            float dataTop = headerY;
            Rectangle scissors = new Rectangle();
            Rectangle clip = new Rectangle(tableX, tableY, tableWidth, tableHeight - HEADER_H);
            com.badlogic.gdx.scenes.scene2d.utils.ScissorStack.calculateScissors(
                camera, viewport.getScreenX(), viewport.getScreenY(),
                viewport.getScreenWidth(), viewport.getScreenHeight(),
                batch.getTransformMatrix(), clip, scissors);
        ...
        if (!saves.isEmpty()) {
            ...
            float visibleH = tableHeight - HEADER_H;
            if (totalContentH > visibleH) {
                ...
                bodyFont.draw(batch, scrollHint,
                    tableX + tableWidth - layout.width - 5f,
                    tableY + 18f);
            }
        }
    }
    ...
    private void update(float delta) {
        ...
        float headerY = tableY + tableHeight - HEADER_H;
        float dataTop = headerY;
        if (mouse.x >= tableX && mouse.x <= tableX + tableWidth
