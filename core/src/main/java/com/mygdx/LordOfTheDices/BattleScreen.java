...
public class BattleScreen implements Screen {
    ...
    public BattleScreen(Assets assets, FightManager manager, int sizeX, int sizeY) {
        ...
        tooltipStyle.font = new com.badlogic.gdx.graphics.g2d.BitmapFont();
        tooltipStyle.font.getData().markupEnabled = true;
    ...
    private void createUI() {
        ...
        for (int i = 0; i < 6; i++) {
            placeholderImage = new Image(lockedDice);
        ...
        diceTable.add(rollAllButton).padBottom(5).expand().row();
    ...
    public void updateCards() {
        ...
        for (int i = 0; i < 5; i++) {
            final int index = i;
            cardSlots[i + 1].clearChildren();
            if (cards[i] == null) {
                cardSlots[i + 1].add(new Image(lockedDice));
            } else {
                cards[i].loadTexture();
                Image addedImage = new Image(cards[i].getTextureRegion());
                addedImage.addListener(new ClickListener() {
                    @Override
                    public void clicked(InputEvent event, float x, float y) {
                        manager.actSelectedCard(cards[index]);
                        switchToFirstView();
                    }
                });
                cardSlots[i + 1].add(addedImage);
            }
        }
    }
